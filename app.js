...
// Interactive tool for visualizing NoC data movement test performance across
// architectures. Fetches CSV results and renders bandwidth/latency plots with
// Plotly, supporting sweep dimension filtering and multiple x-axis selections.

// ── Configuration ──────────────────────────────────────────────────

const YAML_BASE =
    `https://raw.githubusercontent.com/tenstorrent/tt-metal/main/tests/tt_metal/tt_metal/data_movement`;

const DATA_BASE =
    "https://raw.githubusercontent.com/tenstorrent/tt-low-level-documentation/main/data_movement_doc";
...
async function loadYamlConfig() {
    const groupsPath = `${YAML_BASE}/python/test_mappings/web_viewer_groups.yaml`;
    const testInfoPath = `${YAML_BASE}/python/test_mappings/test_information.yaml`;
...
async function probeArchitectures(test) {
    ...
    const probes = KNOWN_ARCHITECTURES.map(async (arch) => {
        ...
        const url = `${DATA_BASE}/perf_plots/${arch}/csv/${csvFile}`;
        try {
...
async function selectTest(group, test) {
    ...
    document.getElementById("readme-link").href =
        `${README_BASE}/${group.directory}/README.md`;
...
async function loadAndRender() {
    ...
    const csvFile = encodeURIComponent(test.csv)
    const url = `${DATA_BASE}/perf_plots/${state.selectedArch}/csv/${csvFile}`;
